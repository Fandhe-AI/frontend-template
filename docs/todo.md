...
- [ ] **Git hooks**
  - [ ] lefthook.yml作成
  - [ ] commitlint.config.ts作成
  - [ ] pre-commit hooks設定（dependency-cruiser含む）
  - [ ] commit message規約
...
- [ ] **Dependency cruiser 高度活用**
  - [ ] アーキテクチャ図の自動生成とドキュメント化
  - [ ] 依存関係メトリクスの収集
  - [ ] 技術負債の可視化
  - [ ] Feature-Sliced Design適合度の定期レポート
  - [ ] カスタムルールの作成（プロジェクト固有制約）

### UI/UX改善

- [ ] **Tailwind CSS導入**
  - [ ] packages/shared/config-tailwind パッケージ作成
  - [ ] デザインシステム統一
  - [ ] 既存コンポーネントの移行
